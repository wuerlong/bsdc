...
if($id>0){
	...
	if($_SESSION['userid']==2){
		$orderlist = $db->findAll("select a.* from phpaadb_order a left join phpaadb_member b on a.tel = b.tel  where  a.pickortime >= '".$timelist[0]."' and a.pickortime <= '".$timelist[29]."' and category = '午餐' and a.carno = '宝教实验学校' group by tel order by b.id asc ");
	}else{
		$orderlist = $db->findAll("select a.* from phpaadb_order a left join phpaadb_member b on a.tel = b.tel  where  a.pickortime >= '".$timelist[0]."' and a.pickortime <= '".$timelist[29]."' and category = '午餐' and a.carno != '宝教实验学校' group by tel order by b.id asc ");
	}
    ...
    <td valign="top" style="padding:10px;"><table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="table_head">
        <tr>
          ...
          <td width="300"><select name="select"
					onChange="window.location.href='report04b.php?id='+this.value">
...
<option value="2021-01">2021-01</option>
	 <option value="2021-02">2021-02</option>
      ...
      <table width="1500" border="1" align="center" cellpadding="2" cellspacing="0" class="table_form table table-bordered table-striped" id="test_table">
        <tr>
          <td colspan="32" align="center"><?php echo $id;?>月份午餐吃饭汇总名单<br>
          (<?php echo $id?>)
          </td>
        </tr>
  ...
  ?>
        <tr>
		  ...
		  foreach ($timelist as $listval){
		   $uulist = $db->findAll("select * from phpaadb_order where uname = '".$list['uname']."' and pickortime = '".$listval."' and category = '午餐' order by pickortime asc ");
		  ?>
  ...
  ?>    </tbody>
      ...
      <table width="100%" border="0" cellpadding="0" cellspacing="0" class="table_footer">
        <tr>
          <td height="29" style="text-align:left; padding-left:10px"><button type="button" class="btn btn-success btn-block" id="generate-excel"><i class="fa fa-file-excel-o" aria-hidden="true"></i> 导出</button>  </div></td>
        </tr>
        <tr>
