...
$orderlist = $db->findAll("select a.* from phpaadb_order a left join phpaadb_member b on a.tel = b.tel  where  a.pickortime = '".$id."' and category = '午餐' order by b.inviteid desc,b.id asc ");
...
// 设置sheet名
$objPHPExcel->getActiveSheet()->setTitle('吃饭汇总名单（午饭）');
...
$outputFileName = '吃饭汇总名单（午饭）_' . time() . '.xls';//导出的文件名
