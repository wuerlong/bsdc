...
if($id>0 && $partment !=''){
	...
	//echo ("select * from phpaadb_order where carno = '".$partment."' and pickortime >= '".$worklist['tel']."' and pickortime <= '".$worklist['worktime']."' group by uname order by id asc ");
	//print_r($orderlist);
    ...
    <td valign="top" style="padding:10px;"><table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="table_head">
        <tr>
          ...
          <td width="300"><select name="select"
					onChange="window.location.href='report02.php?partment=<?php echo $partment;?>&id='+this.value">
					<option value="0">--请选择周--</option>
              <?php getWeekSelect ($id)?>
          </select>&nbsp;&nbsp;<select name="select"
					onChange="window.location.href='report02.php?id=<?php echo $id;?>&partment='+this.value">
  ...
  ?>
        <tr>
          ...
          <?php 
		   ...
		   //cho ("select * from phpaadb_order where uname = '".$list['uname']."' and pickortime >= '".$worklist['tel']."' and pickortime <= '".$worklist['worktime']."' and category ='午餐' order by pickortime asc ");
		   //print_r($uulist);
		//echo $timelistlengh;
