...
if($id>0){
	...
	if($_SESSION['userid']==2){
		$orderlist = $db->findAll("select a.* from phpaadb_order a left join phpaadb_member b on a.tel = b.tel  where  a.pickortime = '".$id."' and category = '午餐' and a.carno = '宝教实验学校' and num > 0 order by b.inviteid desc,b.id asc ");
	}else{
		$orderlist = $db->findAll("select a.* from phpaadb_order a left join phpaadb_member b on a.tel = b.tel  where  a.pickortime = '".$id."' and category = '午餐' and a.carno != '宝教实验学校' and num > 0 order by b.inviteid desc,b.id asc ");
	}
}
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" >
  <tr>
    <td valign="top" style="padding:10px;"><table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="table_head">
        <tr>
          <td height="30">吃饭汇总名单（午饭） 
            &nbsp;&nbsp;&nbsp;</td>
          <td width="300"><select name="select"
					onChange="window.location.href='report03b.php?id='+this.value">
  ...
  ?>
      ...
      <table width="100%" border="1" align="center" cellpadding="2" cellspacing="0" class="table_form table table-bordered table-striped" id="test_table">
        <tr>
          <td colspan="6"><?php echo $partment;?>吃饭汇总名单（午饭） <br>
          (<?php echo $id?>)
          </td>
        </tr>
        <tr>
