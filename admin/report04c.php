...
if($id>0){
	...
	if($_SESSION['userid']==2){
		$orderlist = $db->findAll("select a.* from phpaadb_order a left join phpaadb_member b on a.tel = b.tel  where  a.pickortime >= '".$timelist[0]."' and a.pickortime <= '".$timelist[29]."' and category = '早餐' and a.carno = '宝教实验学校' group by tel order by b.id asc ");
	}else{
		$orderlist = $db->findAll("select a.* from phpaadb_order a left join phpaadb_member b on a.tel = b.tel  where  a.pickortime >= '".$timelist[0]."' and a.pickortime <= '".$timelist[29]."' and category = '早餐' and a.carno != '宝教实验学校' group by tel order by b.id asc ");
	}
	//print_r($orderlist);
    ...
    <td valign="top" style="padding:10px;"><table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="table_head">
        <tr>
          ...
          <td width="300"><select name="select"
					onChange="window.location.href='report04c.php?id='+this.value">
