...
<table width="100%" border="0" cellpadding="0" cellspacing="0" >
  <tr>
    <td valign="top" style="padding:10px;"><table width="100%" border="0" align="center" cellpadding="0" cellspacing="0" class="table_head">
        <tr>
          ...
          <td width="80"><select name="select"
					onChange="window.location.href='report01c.php?id='+this.value">
	...
	foreach ($timelist as $val){
		$uulist = $db->find("select sum(num) as totalnum from phpaadb_order where carno = '宝教实验学校' and type = 0 and pickortime = '".$val."' and category ='早餐' ");
		for ($i=0;$i<$timelistlengh;$i++){
			if($val==$timelist[$i]){
